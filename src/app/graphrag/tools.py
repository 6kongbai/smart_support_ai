...
CYPHER_TEMPLATES: Dict[str, CypherTemplate] = {
    ...
    "products_price_range": CypherTemplate(
        cypher="MATCH (p:Product) WHERE toFloat(p.UnitPrice) >= $min_price AND toFloat(p.UnitPrice) <= $max_price RETURN p.ProductName, p.UnitPrice",
    ...
    "customer_inactive": CypherTemplate(
        cypher="MATCH (c:Customer)-[:PLACED]->(o:Order) WITH c, max(o.OrderDate) as LastOrder RETURN c.CompanyName, LastOrder ORDER BY LastOrder ASC LIMIT 10",
        description="查询【沉睡客户】：列出最近一次下单时间最早的10位客户（可能流失）。",
        required_params=[]
    ),

    # ==================== 3. 订单与物流查询 (Order & Logistics) [重点增强] ====================
    ...
    # [NEW] 物流追踪 (利用 TrackingNumber)
    "track_order": CypherTemplate(
        cypher="MATCH (o:Order) WHERE o.TrackingNumber = $tracking_number RETURN o.orderId, o.Status, o.CurrentLocation, o.ShippedDate",
        description="根据【物流单号】查询包裹当前位置和状态。",
        required_params=["tracking_number"]
    ),
    # [NEW] 物流公司查询
    "check_shipper": CypherTemplate(
        cypher="MATCH (o:Order)-[:SHIPPED_VIA]->(s:Shipper) WHERE o.orderId = $order_id RETURN s.CompanyName, s.Phone",
    ...
    # [NEW] 差评监控 (Risk Control)
    "negative_reviews": CypherTemplate(
        cypher="MATCH (p:Product)<-[:ABOUT]-(r:Review) WHERE toFloat(r.Rating) <= 2 RETURN p.ProductName, r.Rating, r.ReviewText ORDER BY r.ReviewDate DESC LIMIT 10",
    ...
    # [NEW] 折扣商品查询 (利用 Discount 属性)
    "discounted_products": CypherTemplate(
        cypher="MATCH (o:Order)-[c:CONTAINS]->(p:Product) WHERE toFloat(c.Discount) > 0 RETURN distinct p.ProductName, c.UnitPrice, c.Discount ORDER BY c.Discount DESC LIMIT 10",
        description="查询历史上打折力度最大的商品。",
        required_params=[]
    ),

    # ==================== 8. 图谱推荐 (Recommendation) [图数据库核心优势] ====================
    # [NEW] 买了又买
    "recommend_co_purchase": CypherTemplate(
        cypher="MATCH (p:Product {ProductName: $product_name})<-[:CONTAINS]-(:Order)-[:CONTAINS]->(reco:Product) WHERE reco.ProductName <> $product_name RETURN reco.ProductName, count(*) as Freq ORDER BY Freq DESC LIMIT 5",
